(() => {
  ...
  document.addEventListener('click', event => { if (!mobileQuery.matches || state !== 2 || drawer.contains(event.target)) return; blurField(); keyboardGrace(isAndroid ? 1000 : 700); event.preventDefault(); event.stopPropagation(); closeDrawer(); }, true);
  ...
  document.addEventListener('keydown', event => { if (event.key === 'Escape' && state === 2) closeDrawer(); });
  ['wheel', 'touchmove'].forEach(eventName => document.addEventListener(eventName, event => { if (state === 2 && !drawer.contains(event.target)) event.preventDefault(); }, { passive: false, capture: true }));
