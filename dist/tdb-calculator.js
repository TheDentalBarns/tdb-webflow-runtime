/* TDB Treatment Calculator v1.4.6 — deterministic pricing and planning rules. */
...
(function () {
  ...
  class View {
    ...
    option(o,e){
      ...
      const choices=complexity?'<fieldset class="tdbc-complexity"><legend class="text-size-small">'+(o.key==='aligners'?'What would you like to improve?':'What changes do you have in mind?')+'</legend>'+r.tierLabels.map((label,i)=>label&&r.tiers[i]?'<label class="tdbc-choice form_checkbox text-size-small"><input type="radio" name="'+rid+'-complexity" data-control="tier-'+o.key+'-'+i+'" data-tier="'+o.key+'" value="'+i+'" '+(v?.tier===i?'checked':'')+'>'+check(v?.tier===i)+'<span>'+esc(label)+(o.key==='aligners'&&r.tierTimings[i]?' · '+(r.tierTimings[i].min===r.tierTimings[i].max?r.tierTimings[i].min:r.tierTimings[i].min+'–'+r.tierTimings[i].max)+' months':'')+'</span></label>':'').join('')+'<p class="tdbc-help text-size-tiny">Choose the closest description. Your dentist will confirm what’s right for you.</p></fieldset>':'';
