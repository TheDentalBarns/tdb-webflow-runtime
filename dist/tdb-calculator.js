/* TDB Treatment Calculator v1.4.1 — deterministic pricing and planning rules. */
...
(function () {
  ...
  const actionButton=(action,label,up=false,alternate=false,withArrow=true)=>'<button type="button" class="button '+(withArrow?'is-icon ':'')+'is-secondary '+(alternate?'is-alternate ':'')+'w-inline-block" data-action="'+action+'"><span>'+label+'</span>'+(withArrow?'<span class="icon-embed-xxsmall '+(up?'is-up ':'')+'w-embed">'+arrow+'</span>':'')+'</button>';
  ...
  class View {
    ...
    showTimingWarning(){this.timingWarning=true;const p=this.root.querySelector('.tdbc-timing-warning');if(p)p.hidden=false;}
    dismissTimingWarning(){this.timingWarning=false;const p=this.root.querySelector('.tdbc-timing-warning');if(p)p.hidden=true;}
    ...
    click(event){const el=event.target.closest('[data-action]');if(!el)return;const action=el.dataset.action;
      ...
      if(action==='sooner'){this.showTimingWarning();return;}
      if(action==='dismiss-timing'){this.dismissTimingWarning();return;}
      if(action==='bridal'){this.bridalOpen=!this.bridalOpen;this.render();return;}
