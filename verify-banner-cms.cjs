...
function setup({path='/',embedded=true,saved=true,mobile=false,reduced=false,animationDelay=400,fields=fieldValues}={}){
 ...
 w.HTMLElement.prototype.animate=function(){const animation={onfinish:null,cancel(){w.clearTimeout(handle)}};const handle=w.setTimeout(()=>animation.onfinish?.(),animationDelay);return animation;};
...
(async()=>{
 ...
 console.log('PASS: touch capture handover, reduced-motion direct drag, delayed animation settlement, CMS BST conversion, missing-hour rejection, verbatim CMS titles/actions, left-slide loop, hover pause, mobile drawer route, Signature-only route, hidden/pagehide pause, consent-gated one settings read.');
