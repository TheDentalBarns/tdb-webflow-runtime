...
Validation: five Node tests cover relevance ordering, exact snippet entry across cross-posted sources, explicit topic ranking, newest/filter/dedup behaviour and safe source URLs. A staging-only `?review-preview=mobile` layout mode constrains the real panel to 390 by 844 CSS pixels for narrow-layout inspection; it is not device emulation. Staging browser checks confirmed opening, arrows, pointer swipe, topic/order/source filters, Doctify SVG/history, Yell unrated labels, internal Read more scrolling, and close restoring focus and the exact page scroll position. Desktop and 390-pixel panel layouts were visually inspected. Real touch-device validation remains a separate check. Rollback: restore both v1.0.7 power embeds; the inert drawer-data embeds can remain or be removed. No custom domains are published.

Final ordering refinement: badge entry uses full-review rank for the page's context; only snippet entry pins an exact record. Selecting a topic overrides the page context and snippet preference, using that topic's dedicated rank when present. Drawer DOM order follows the visible header, review and navigation order.

### v1.2.0 — Gallery animation and simplified reading layout

User-directed visual refinement: card, top bar, bottom controls and X morph now start together, removing the fixed 420ms control delay. The X uses Smile Gallery's exact separate quintic movement/rotation timing (400/600ms), 200ms middle-line collapse and reversed close sequence. The drawer always animates, including when reduced motion is requested, as explicitly requested for this preview.

Removed score information, topic/source/order controls, Read more/Read less, the text fade and external source links from the drawer. Contextual ranking and exact-snippet entry remain. Each review is one vertically scrollable card with the complete review text; the top bar uses the Gallery's 20px backdrop blur, 150% saturation and 84% cream background. Bottom controls remain fixed and per-review scroll positions are retained between slides.

The reserved quote area remains consistent between slides. Its quotation mark occupies a fixed left column and the statement is left aligned beside it. Reviewer name and date are left aligned beneath; the date reuses the Gallery's clock SVG. The same row places the grayscale platform vector and cream stars at bottom right. The cream uses the existing orange-3 palette token (#d6cab4). Verified individual ratings remain accurate; this revision does not turn unrated or four-star records into five-star reviews. All changes remain confined to the staging preview.

### v1.3.0 — Cream quote panel and stationary ornament

The short excerpt now occupies a dark cream orange-3 panel inset by the existing drawer gutters, leaving light cream visible at each side. One shared light cream quotation mark is centred above the statement outside the horizontal slide track; it and the panel remain stationary while reviews move. The panel follows vertical review scrolling so continuous reading beneath the glass header is preserved. Its fixed reserved height is 14rem to accommodate the ornament, text and internal padding consistently.

Only the short excerpt fades: 200ms ease-out, matching the service parallax slider, revealed 100ms after forward settlement or 140ms after backward settlement (60ms after a cancelled drag). Initial reveal waits for the card entrance. Text is hidden during replacement to avoid flashing the incoming excerpt. The drawer heading uses the existing cream token. Names, dates, ratings, full review text, navigation, ranking and staging-only boundary are unchanged.
