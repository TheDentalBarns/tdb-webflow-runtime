...
Automated tests cover 37 scenarios across pricing, bundling, restrictions, persistence, context conflicts, VIP handoff, text escaping, finance and date boundaries. See `validation.md` for actual staged-browser evidence and release pins.
...
The target completion card starts with the shortest estimated duration, including the two-week assessment lead-in. It keeps the longer finish date visible for uncertain ranges, and never implies diary availability. The slider explores up to two years of additional delay; a date input supports direct selection within that interval. A leftward attempt at the minimum opens an inline deadline message. The asterisk explains that dates are a rough guide, with healing, refinements and appointment availability affecting the confirmed plan. Missing timing fields use documented planning defaults and disclose that allowance. The full-width deadline panel opens directly beneath the arrow-free Need it sooner button. Wedding advice expands inside that panel using the existing chevron animation. A hidden, aria-hidden full-length completion message reserves responsive space so the short/long variants do not shift the slider.
...
The live amount uses a subtle 240ms fade with a 2px movement when its value changes; reduced-motion preferences skip this. Timeline rows begin at 50% opacity, with the stage nearest 42% of the viewport brought to full opacity on scroll. Tapping a row highlights and opens it without scrolling; the next scroll resumes position-based emphasis. Only the bottom action pair remains. Restart retains the native up arrow; assessment-only has no arrow. The footnote text size is scoped to the paragraph so both buttons inherit normal native typography.
