...
(function () {
  ...
  const today=()=>{const p=Object.fromEntries(new Intl.DateTimeFormat('en-GB',{timeZone:'Europe/London',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date()).map(x=>[x.type,x.value]));return p.year+'-'+p.month+'-'+p.day;};
  ...
  const availability={slot:null,checked:0,pending:null,error:false,timer:0};
  ...
  function updateAvailability(){
    ...
    if(slot){const next=freshUntil>Date.now()?Math.min(freshUntil,slot.at):slot.at;availability.timer=setTimeout(updateAvailability,Math.max(1,Math.min(2147483647,next-Date.now()+1)));}
    if(records)for(const view of views)if(view.ready)view.outputs();
  }
  ...
  class View {
    ...
    timelineOutput(e){
      ...
      for(const stage of stages){
        ...
        const detail='<div id="'+panelId+'" class="tdbc-stage-detail"><p>'+esc(stage.description)+'</p>'+lines.map(l=>'<div class="tdbc-stage-cost"><span>'+esc(l.label+lineQuantity(l))+(Number.isInteger(l.tier)&&l.record?.tierLabels?.[l.tier]?'<small class="text-size-tiny">'+esc(l.record.tierLabels[l.tier])+'</small>':'')+'</span><strong>'+(l.included?'Included':l.min===null?'To confirm':esc((l.from||l.record?.tiers?.length?'From ':'')+range(l.min,l.max)))+'</strong></div>').join('')+(stage.sharedFee?'<p class="tdbc-help text-size-tiny">Your total treatment fee, counted once in the estimate; preparation and fitting are included.</p>':!lines.length?'<p class="tdbc-help text-size-tiny">Planning allowance · no additional treatment fee.</p>':'')+'</div>';
    ...
    change(event){
      ...
      else if(el.dataset.date==='target'){const t=C.completionTimeline(records,C.estimate(records,state,this.config),planningStart(),state.delay,el.value);state.delay=t.offset;if(t.tooSoon)this.showTimingWarning();else this.dismissTimingWarning();}
    ...
    click(event){const el=event.target.closest('[data-action]');if(!el)return;const action=el.dataset.action;
      if(action==='availability'){getAvailability(true);return;}
  ...
  function start(){
    addEventListener('online',()=>{if(views.length)getAvailability(true);});
    document.addEventListener('visibilitychange',()=>{if(!document.hidden&&views.length){updateAvailability();getAvailability();}});
    addEventListener('pageshow',event=>{if(event.persisted&&views.length){updateAvailability();getAvailability();}});
