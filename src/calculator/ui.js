...
(function () {
  ...
  class View {
    ...
    render(){
      ...
      ('<div class="tdbc-live '+(this.mode==='inline'?'padding-global ':'')+(hasEstimate?'has-estimate':'')+'" data-node="live" aria-hidden="'+!hasEstimate+'" '+(hasEstimate?'':'inert')+'><button type="button" class="tdbc-live-button container-large" data-action="estimate" aria-label="View your full estimate"><span class="tdbc-live-copy"><span class="text-size-tiny">Your estimate</span><span class="tdbc-live-value text-size-small">'+tag+'<strong data-output="live">'+esc(priceText(e))+'</strong></span></span><span class="tdbc-live-copy"><span class="text-size-tiny">Estimated duration</span><span class="tdbc-live-value text-size-small">'+clock+'<span data-output="duration">'+esc(C.duration(records,e,today()))+'</span></span></span><span class="icon-embed-xxsmall is-down w-embed tdbc-scroll-arrow">'+arrow+'</span></button>'+(this.mode==='drawer'?'<button type="button" class="tdbc-live-close" data-action="close" aria-label="Close calculator">×</button>':'')+'</div>')+'<div class="tdbc-main" data-node="main">'+form+'</div></div><footer class="tdbc-section-footer" data-node="section-footer"><div class="container-large"><div class="max-width-xsmall"><p class="text-style-tagline-restored tdbc-dd-fade">We believe everyone deserves to feel good about their smile</p></div></div></footer>'+(this.mode==='inline'?'<section class="tdbc-price-guide container-large padding-section-large" data-node="price-guide-intro"><header class="tdbc-header"><p class="text-style-tagline-restored tdbc-dd-fade">CLEAR PRICES, CONSIDERED CARE</p><h2 class="heading-style-h3">Your full treatment price guide</h2><p class="text-size-small">Explore our fees treatment by treatment, from everyday care to a change in your smile.</p></header></section>':''));
      this.outputs();
    }
    option(o,e){
