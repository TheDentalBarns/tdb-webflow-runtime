...
(() => {
    ...
    const isTransparent = navbar.getAttribute("transparent-nav") === "true";
    const mobileQuery = matchMedia("(max-width:767px)");
    const revealDistance = 120;
    const hideDistance = 140;
    let viewportWidth = innerWidth;
    let protectedZone = innerHeight * .75;
    let scrollTop = Math.max(scrollY || root.scrollTop || 0, 0);
    let lastScrollTop = scrollTop;
    let upwardDistance = 0;
    let downwardDistance = 0;
    let focusState = null;
    let transparentState = 0;
    let hoverState = 0;
    let dropdownState = 0;
    let menuOpenState = 0;
    ...
    let appliedTransparentState = null;
    let animationFrame = 0;
    let currentTransform = "";
    let zHoldTimer = 0;
    let transparentElements = [];
    let atTop = scrollTop <= 4;
    const isMobileTransparent = () => isTransparent && mobileQuery.matches;
    const isProtectedZone = value => isMobileTransparent() && value < protectedZone;
    const isHidden = () => currentTransform.includes("-100%");
    // Sliders supply intent. This controller owns the scroll thresholds and the
    ...
    function collectTransparentElements() {
        transparentElements = [ ...document.querySelectorAll("#nav-button-link,.navbar10_component,.navbar10_logo-link,.navbar10_link,.menu-icon5") ];
    }
    if (isTransparent) collectTransparentElements();
    function setTransform(value) {
        if (value === currentTransform) return;
        currentTransform = value;
        navbar.style.transform = value;
    }
    function updateTransparentState() {
        if (!isTransparent) return;
        const value = transparentState || hoverState || dropdownState || menuOpenState;
        if (value === appliedTransparentState) return;
        appliedTransparentState = value;
        if (!transparentElements.length) collectTransparentElements();
        transparentElements.forEach(element => {
            element.classList.toggle("is-trans", value);
        });
    }
    function withoutMotion(callback) {
        const previousTransition = navbar.style.transition;
        navbar.style.transition = "none";
        callback();
        navbar.offsetHeight;
        if (previousTransition) navbar.style.transition = previousTransition; else navbar.style.removeProperty("transition");
    }
    function setAbsolute() {
        if (!isMobileTransparent()) return;
        withoutMotion(() => {
    ...
    function revealNavbar() {
        if (!isMobileTransparent()) return setTransform("translateY(0)");
        transparentState = 1;
        ...
        fixedState = 1;
        requestAnimationFrame(() => setTransform("translateY(0)"));
    }
    function updateTopState() {
        if (!isTransparent) return;
        if (!mobileQuery.matches) atTop = false; else if (scrollTop <= 4) atTop = true; else if (scrollTop > 24) atTop = false;
        root.classList.toggle("tdb-nav-at-top", !!atTop);
    }
    function updateScroll() {
        scrollTop = Math.max(scrollY || root.scrollTop || 0, 0);
        const delta = scrollTop - lastScrollTop;
        const insideProtectedZone = isProtectedZone(scrollTop);
        if (focusState?.isDragging()) upwardDistance = downwardDistance = 0;
        else if (delta > 0) {
            upwardDistance = 0;
        ...
        const releaseFocus = focusState && (upwardDistance > revealDistance ||
            downwardDistance > hideDistance || (scrollTop <= 40 && delta < 0)) ? focusState.onRelease : null;
        updateTopState();
        if (scrollTop <= 0) {
        ...
        if (isMobileTransparent()) {
            if (insideProtectedZone) {
                if (!focusState) upwardDistance = downwardDistance = 0;
                if (fixedState && isHidden()) setAbsolute();
            } else if (menuOpenState || transitionLock) {
                if (fixedState) setTransform("translateY(0)"); else revealNavbar();
                if (!focusState) upwardDistance = downwardDistance = 0;
            } else if (delta > 0) {
                if (fixedState) {
                    if (downwardDistance > hideDistance) setTransform("translateY(-100%)");
                } else if (!focusState) {
                    downwardDistance = 0;
                }
            } else if (delta < 0) {
            ...
            if (scrollTop > 100) transparentState = 1;
            if (upwardDistance > revealDistance) setTransform("translateY(0)");
        }
        lastScrollTop = scrollTop;
        updateTransparentState();
        animationFrame = 0;
    ...
    mobileQuery.addEventListener ? mobileQuery.addEventListener("change", handleBreakpointChange) : mobileQuery.addListener(handleBreakpointChange);
    if (isTransparent) {
        ...
        function unlockMenuTransition() {
            clearTimeout(menuTransitionTimer);
            menuTransitionTimer = null;
            transitionLock = 0;
            navbar.classList.remove("tdb-menu-transitioning");
        }
        function startMenuTransition(event) {
            if (transitionLock) {
        ...
        [ menuButton, menu, overlay ].filter(Boolean).forEach(element => {
    ...
    if (isMobileTransparent()) navbar.style.position = "absolute";
    setTransform("translateY(0)");
    updateScroll();
})();
...
(() => {
    const initMobileNavTextClose = () => {
        ...
        const animateTextOut = () => {
            ...
            clearTextAnimations();
            menu.querySelectorAll(".navbar10_menu-left, .navbar10_menu-right").forEach(element => {
        ...
        button.addEventListener("pointerdown", () => {
            ...
            if (menuIsOpen()) animateTextOut(); else clearTextAnimations();
        }, {
            capture: true,
            passive: true
        });
    ...
    if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", initMobileNavTextClose, {
            once: true
        });
    } else {
